...
export function getKingStartSquare(side) {
  return side === "black" ? "e8" : "e1"
}

function getLastRank(side) {
  return side === "black" ? 1 : 8;
}

export function isPromotionMove(move, side) {
  if (move.piece.toLowerCase() !== "p") return false;
  const [_, rank] = move.dest;
  return parseInt(rank) === getLastRank(side);
}
...
export function isCastlingMove(event, piece) {
  return (
    piece.toLowerCase() === "k" 
    && (event.squareFrom[0] === "e" 
      && ["g", "c"].includes(event.squareTo[0]))
  );
}

export function isEnPassantMove(move, board, side) {
  if (!move.dest) {
    return false;
  }
