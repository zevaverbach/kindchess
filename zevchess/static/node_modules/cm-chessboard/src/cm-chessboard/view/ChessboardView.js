...
export class ChessboardView {
  ...
  drawDots() {
    while (this.markersTopLayer.firstChild) {
      this.markersTopLayer.removeChild(this.markersTopLayer.firstChild)
    }
    this.chessboard.state.dots.forEach(square => this.drawDot(square.square))
  }

  drawDot(square) {
    const markersTopLayer = Svg.addElement(this.markersTopLayer, "g")
    markersTopLayer.setAttribute("data-square", square)
    const point = this.squareToPoint(square)
    const transform = (this.svg.createSVGTransform())
    transform.setTranslate(point.x, point.y)
    markersTopLayer.transform.baseVal.appendItem(transform)
    const spriteUrl = this.chessboard.props.sprite.cache ? "" : this.chessboard.props.sprite.url
    const markerUse = Svg.addElement(markersTopLayer, "use",
      { href: `${spriteUrl}#markerDot`, class: "marker marker-dot" })
    const transformScale = (this.svg.createSVGTransform())
    transformScale.setScale(this.scalingX, this.scalingY)
    markerUse.transform.baseVal.appendItem(transformScale)
    return markersTopLayer
  }
