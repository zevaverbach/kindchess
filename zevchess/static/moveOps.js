...
export function doTheMoveSentCastle(move, board) {
    const rank = store.side === "black" ? 8 : 1; // it's this side that castled
    if (move.castle === "k") {
...
export function doTheMoveSentEnPassant(move, board) {
    const [file, rankStr] = move.dest;
    const rank = parseInt(rankStr);
    const sq = store.side === "black" ? `${file}${rank + 1}` : `${file}${rank - 1}`;
    board.setPiece(sq, null);
}

export function doTheMoveReceived(move, board) {
    let from, to;
    if (move.castle) {
        const rank = store.side === "black" ? 1 : 8; // it's the other side that castled
        if (move.castle === "k") {
